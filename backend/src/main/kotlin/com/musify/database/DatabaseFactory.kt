...
object DatabaseFactory {
    ...
    fun init() {
        ...
        if (EnvironmentConfig.DATABASE_URL.contains("postgresql")) {
            println("INFO: Initializing PostgreSQL database...")
        ...
        transaction(_database) {
            if (EnvironmentConfig.ENVIRONMENT != "production") {
                ...
                SchemaUtils.create(
                    ...
                    // Offline Downloads
                    OfflineDownloads,
                    OfflineDownloadQueues,
                    UserOfflineSettings,
                    DeviceDownloadLimits,
                    OfflinePlaybackSessions,
                    OfflineDownloadAnalytics,
                    OfflineDownloadBatches
                )
                println("INFO: Database schema created successfully")
            } else {
