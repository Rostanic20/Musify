    ...
    private suspend fun executeCleanupActions(actions: List<CleanupAction>): StorageCleanupResult {
        ...
        for (action in actions) {
            try {
                ...
                // Cleanup action failed; continuing with remaining actions
            }
        }
    ...
    private suspend fun cleanupExpiredDownloads(userId: Int, deviceId: String) {
        ...
        for (download in expiredDownloads) {
            try {
                ...
                // Expired download cleanup failed; continuing with remaining downloads
            }
        }
    }
        ...
        if (settings.autoDeleteAfterDays > 0) {
            ...
            for (download in oldDownloads) {
                try {
                    ...
                    // Old download cleanup failed; continuing with remaining downloads
                }
            }
        }
    }
        ...
        cleanupScope.launch {
            while (true) {
                try {
                    ...
                    // Automatic cleanup cycle failed; will retry on next interval
