...
class ScheduledTasks(
    ...
    private suspend fun performDatabaseBackup() {
        when (val result = databaseBackupService.performBackup()) {
            is Result.Success -> {
                SentryConfig.addBreadcrumb(
                    message = "Database backup completed",
                    category = "backup",
                    data = mapOf("key" to result.data)
                )
            }
            is Result.Error -> {
                SentryConfig.captureMessage(
    ...
    private suspend fun cleanupOldBackups() {
        when (val result = databaseBackupService.cleanupOldBackups()) {
            is Result.Success -> {
                SentryConfig.addBreadcrumb(
                    message = "Backup cleanup completed",
                    category = "backup",
                    data = mapOf("deleted" to result.data)
                )
            }
            is Result.Error -> {
                SentryConfig.captureMessage(
                    "Backup cleanup failed: ${result.message}",
                    com.musify.core.monitoring.SentryLevel.ERROR
                )
            }
        }
    ...
    private suspend fun cleanupExpiredCaches() {
        // Placeholder: cache cleanup is handled by Redis TTL policies
    }
}
